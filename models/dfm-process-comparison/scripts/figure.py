...
fig=plt.figure(figsize=(17,11),facecolor='#f4f5f7')
fig.text(.055,.952,'One part. Four manufacturing paths.',fontsize=26,weight='bold',color='#172638')
fig.text(.055,.916,'Same 80 × 60 × 25 mm envelope · four Ø4.5 holes at mounting datum',fontsize=13,color='#536273')
...
cards.append(('injection','04 / Injection molding','#93b285','2.4 mm floor · 1° draft · thin ribs','Drafted walls, ribs and holes; resin/tooling still to review.'))
for i,(key,title,color,spec,caption) in enumerate(cards):
 col=i%3; row=i//3; left=.045+col*.325; bottom=.49-row*.405
 ax=fig.add_axes([left,bottom,.29,.31])
 ...
 fig.text(left,bottom+.322,title,fontsize=15,weight='bold',color='#172638')
 fig.text(left,bottom-.012,spec,fontsize=10,weight='bold',color='#25384b')
 fig.text(left,bottom-.038,caption,fontsize=8,color='#536273')
...
fig.text(.695,.36,'What stays fixed',fontsize=17,weight='bold',color='#172638')
fig.text(.695,.325,'80 × 60 × 25 mm envelope\nFour mounting-hole positions\nØ4.5 mm at the underside datum',fontsize=12,linespacing=1.8,color='#536273',va='top')
fig.text(.695,.17,'Molding concept',fontsize=17,weight='bold',color='#172638')
fig.text(.695,.14,'Assumed pull along Z.\nWall thickness varies with draft.\nNo mold-flow or ejection validation.',fontsize=12,linespacing=1.8,color='#536273',va='top')
