...
from build123d import Box, Cylinder, Location, Rectangle, extrude
...
def mx_socket_cutter(plate_top_z: float = 0.0, pocket_depth: float = MX_POCKET_DEPTH):
    ...
    ).moved(Location((0.0, 0.0, plate_bottom_z - pocket_depth / 2.0)))
    ...
    ).moved(Location((0.0, 0.0, plate_top_z - MX_PLATE / 2.0 - 0.5)))

    # 45 degree funnel above the plate top (negative taper widens upward)
    lead = extrude(Rectangle(HOLE_SIDE, HOLE_SIDE), MX_LEAD_IN, taper=-45.0).moved(
        Location((0.0, 0.0, plate_top_z))
    )
    ...
    grooves = [
        Cylinder(MX_RELIEF_RADIUS, groove_height).moved(
            Location((offset_x, offset_y, groove_center_z))
        )
