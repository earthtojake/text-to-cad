"""Refresh the embedded material-only braid presentation source."""
import sys,json
...
source_path=root/'src/hand_progress_review.py'
source=source_path.read_text();marker="ANIMATION_JS = r'''";start=source.index(marker)+len(marker);end=source.index("\n'''",start)
source_path.write_text(source[:start]+script.rstrip()+source[end:])
...
print('Refreshed48rope embedded static presentation')
