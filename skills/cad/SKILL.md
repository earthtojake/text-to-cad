...
5. **Plan before coding.** Define parameters, intent labels, source paths, expected bounding boxes, and any mating/positioning datums before editing. When the design includes a moving joint (a hinge, pivot, slider, or rotating stage), identify its type and stated range as part of this plan — that range is what step 8's interference check has to sweep, not just the rest pose.
...
8. **Validate geometrically.** Run `scripts/inspect refs <step-or-cad-target> --facts --planes --positioning` as the baseline, then verify the dimensions and relationships the user's spec calls out with targeted `measure`, `align`, `frame`, or `diff` checks. Run `scripts/inspect validate <step-or-cad-target>` for geometry soundness: `refs --facts` reports counts and bounds, and its `ok` field covers ref resolution only — an open shell and an inverted solid both pass it. For any assembly with more than one positioned part, also run `scripts/inspect interfere <step-or-cad-target>` — `refs`/`validate` never test one part against another, so two solids can be individually valid and still occupy the same space. Re-run it after any positioning or joint-angle change, not only at the pose the assembly happened to be built at. When step 5 identified a moving joint, run `interfere` at representative poses across its stated range — both limits at minimum — as part of this same validation pass, not only reactively after a later change; see `references/inspection-and-validation.md` for what counts as adequate coverage. A clean result at one pose does not show the joint can reach the rest of its range at all.
