...
7. **Generate explicit targets.** Run each model script directly (`python <model>.py`); do not sweep directories. Every run writes the model's `.step` (sibling `<stem>.step` by default; `out=` in the decorator relocates it); declare `@stl`/`@threemf`/`@glb` exports on the model, or run `cadgen stl|3mf|glb build` for one-off mesh files. A model may declare ONLY mesh outputs — `@stl`/`@threemf`/`@glb` with no `@step` is a full model (same tree, record and build) that writes no STEP; there is no requirement to write one. For multi-model project structure, see the `$cad-project` skill.
