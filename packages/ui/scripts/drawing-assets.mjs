...
const SDK_CHUNK = /[\\/]@excalidraw[\\/]excalidraw[\\/]dist[\\/](dev|prod)[\\/][^\\/]+\.js$/;

function assertExclude(exclude) {
  if (!Array.isArray(exclude) || exclude.some(pattern => !(pattern instanceof RegExp))) {
    throw new TypeError("drawing assets `exclude` must be an array of RegExp");
  }
}

/**
 * @param {{ exclude?: RegExp[] }} [options] `exclude` patterns are tested
 *   against each emitted `fileName`; a match is neither served nor emitted.
 *   The localized fallback keeps a miss local: a 404, then a system font.
 */
export function drawingAssetFiles({ exclude = [] } = {}) {
  ...
  // A stateful (g/y) pattern must not carry lastIndex between file names.
  return files.filter(file => !exclude.some(pattern => { pattern.lastIndex = 0; return pattern.test(file.fileName); }));
}
...
/**
 * @param {{ exclude?: RegExp[] }} [options] See `drawingAssetFiles`.
 * @returns {import("vite").Plugin}
 */
export function drawingAssetsPlugin({ exclude = [] } = {}) {
  assertExclude(exclude);
  let files;
  return {
    ...
    config() {
      // Vite 8 optimizes dependencies with Rolldown and only tolerates the
      // esbuild option behind a deprecation warning; give it a native plugin.
    ...
    buildStart() { files = drawingAssetFiles({ exclude }); },
    transform(code, id) {
    ...
    configureServer(server) {
      const assets = new Map(drawingAssetFiles({ exclude }).map(file => [`/${file.fileName}`, file]));
      server.middlewares.use((request, response, next) => {
        const pathname = new URL(request.url ?? "/", "http://localhost").pathname;
        const file = assets.get(pathname);
        if (!file) {
          // A missing or excluded asset is a 404, never the SPA's index.html
          // at 200: the SDK reads a failed status, not an undecodable font.
          if (!pathname.startsWith(`/${DRAWING_ASSET_DIRECTORY}/`)) return next();
    ...
    generateBundle() {
      for (const file of files ?? drawingAssetFiles({ exclude })) this.emitFile({ type: "asset", ...file });
    },
  };
}
