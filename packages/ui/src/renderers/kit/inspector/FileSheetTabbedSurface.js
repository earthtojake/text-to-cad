...
import { resolveActiveFileSheetSectionId } from "./activeSection.js";
...
function FileSheetTabContent({ section, active }) {
  ...
  return <TabsContent value={section.id} forceMount hidden={!active} className={cn("min-h-0 flex-1 overflow-hidden", active && "flex flex-col")} data-file-sheet-tab-panel={section.id}>
...
export default function FileSheetTabbedSurface({ sections, openSectionIds = [], onOpenSectionIdsChange, headerActions = null }) {
  ...
  return (
    <Tabs value={activeId} onValueChange={id => onOpenSectionIdsChange?.([id])}
      ...
      {/* One section is still a tab: every Inspector reads the same way, whatever the file offers. */}
      <TabsList aria-label="Inspector panels"
