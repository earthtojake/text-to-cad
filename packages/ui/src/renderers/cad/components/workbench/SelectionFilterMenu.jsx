...
export default function SelectionFilterMenu({ value, onChange, disabled, compact, fullWidth, options = SELECTION_FILTERS, menuLabel = 'Selection filter', hint = 'Shift-click adds or removes from selection.' }) {
  ...
  return <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <button type="button" aria-label={`${menuLabel}: ${label}`} title={`${menuLabel}: ${label}`} disabled={disabled}
        ...
        <ListFilter className="size-3 shrink-0" aria-hidden="true" />{(compact || fullWidth) && label}{fullWidth && <ChevronDown className="ml-auto size-3 text-muted-foreground" aria-hidden="true" />}
      </button>
    </DropdownMenuTrigger>
