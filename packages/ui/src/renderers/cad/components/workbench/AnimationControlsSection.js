...
import { FILE_SHEET_PRECISION_SLIDER_CLASSES } from "./FileSheet.js";
...
// Their evaluators remain separate; the bar only edits clip and clock state.
...
function AnimationTimeControl({ playing, elapsedSec, duration, onScrub, clockKind = "step", disabled = false }) {
  ...
  return <div className={cn("flex min-w-0 flex-1 items-center gap-1", compact ? "h-6" : "h-7 px-2",
    ...
    <div className={cn("min-w-0 flex-1 px-1", responsive && "@max-[8rem]/cad-viewport:order-last @max-[8rem]/cad-viewport:basis-full")}>
      <AnimationTimeControl playing={runtime?.playing === true} elapsedSec={runtime?.elapsedSec}
        duration={duration} onScrub={runtime?.onScrub} clockKind={runtime?.clockKind} disabled={disabled}/>
    </div>
  </div>;
}
