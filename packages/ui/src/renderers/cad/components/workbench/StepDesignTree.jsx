...
export default function StepDesignTree({ client, file, revision, label, onOpenFile }) {
  ...
  return <div className="flex h-full min-h-0 flex-col" aria-label="Design features">
        ...
        {state.status !== 'loading' && rows.map((node, index) => {
          ...
          return <div key={node.id} role="treeitem" aria-level={node.depth + 1} aria-expanded={hasChildren ? open : undefined} aria-selected={selected.id === node.id}
            ...
            onKeyDown={event => {
              ...
              if (next !== null) { event.preventDefault(); const target = Math.max(0, Math.min(rows.length - 1, next)); setSelectedId(rows[target].id); elements[target]?.focus(); }
              if ((event.key === 'ArrowRight' && hasChildren && !open) || (event.key === 'ArrowLeft' && hasChildren && open)) { event.preventDefault(); toggle(node.id); }
              else if (event.key === 'ArrowRight' && hasChildren && open) { event.preventDefault(); setSelectedId(rows[index + 1].id); elements[index + 1]?.focus(); }
              else if (event.key === 'ArrowLeft' && node.depth > 0) { event.preventDefault(); const parentIndex = rows.findLastIndex((row, i) => i < index && row.depth < node.depth); setSelectedId(rows[parentIndex].id); elements[parentIndex]?.focus(); }
              if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); if (hasChildren) toggle(node.id); }
            }}>
            {hasChildren ? <button type="button" tabIndex={-1} aria-label={`${open ? 'Collapse' : 'Expand'} ${node.label}`} className="grid size-6 shrink-0 place-items-center rounded-sm hover:bg-sidebar-accent" onClick={event => { event.stopPropagation(); setSelectedId(node.id); toggle(node.id); }}>
    ...
    {selected.type !== 'model' && <section aria-label="Feature properties" className="max-h-[32%] shrink-0 overflow-y-auto border-t border-sidebar-border/70 px-3 py-2.5">
      ...
      {selected.line && <p className="mt-3 text-micro text-muted-foreground">Source line {selected.line} · No face mapping</p>}
    </section>}
    <div className="shrink-0 border-t border-sidebar-border/60 px-3 py-2 text-micro text-muted-foreground">
      {state.source ? <><button className="max-w-full truncate text-left underline-offset-2 hover:underline" title={state.source} onClick={() => onOpenFile?.(state.source)}>{state.source.split('/').pop()}</button><p className="mt-1">Source order, not evaluated build history. Source may differ from this STEP.</p></> : <p>{state.status === 'ambiguous' ? 'Multiple source candidates. No feature history was assumed.' : state.status === 'loading' ? 'Reading matching source without running it.' : 'No source feature history available.'}</p>}
    </div>
  </div>;
}
