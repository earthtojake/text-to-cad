// The render-only portion of View: quality, exposure, lighting and backdrop.
...
import { RENDER_QUALITY, RENDER_QUALITY_PRESETS } from "@hardcore/core/common/sceneSettings.js";
...
export default function RenderSettingsContent({
  ...
  const selectedQuality = payload.quality || RENDER_QUALITY.PREVIEW;
  const effectiveQualityLabel = QUALITY_OPTIONS.find(({ value }) => value === selectedQuality)?.label || "Preview";
  const setValue = (path, value) => onPayloadValueChange?.(path, value);

  return (
    <div data-cad-render-settings-section="true">
      <FileSheetSubsection title="Render">
        <FileSheetSelectRow
          label="Quality"
        ...
        <FileSheetColorRow label="Backdrop" value={configuration.backdrop.color} disabled={configuration.backdrop.transparent} onChange={(value) => setValue(["backdrop", "color"], value)} />
