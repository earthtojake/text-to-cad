...
test('the shared CAD renderer resolves deferred files, reuses warm assets and restores isolated view state', async (t) => {
  ...
  await page.waitForFunction(() => Object.values(window.cadHarness.state.renderers || {}).some(saved => saved.fileSession?.slices?.display?.floor?.enabled));
  ...
  const settledSceneSyncs = () => page.evaluate(async () => {
    for (let count = -1; count !== window.__cadSceneSync.count;) {
      count = window.__cadSceneSync.count;
      await new Promise(resolve => setTimeout(resolve, 300));
    }
    return window.__cadSceneSync.count;
  });
  ...
  await page.waitForFunction(() => Object.values(window.cadHarness.state.renderers || {}).some(saved => saved.fileSession?.slices?.display?.grid?.opacity === 0.4));
  await page.keyboard.press('Escape');
  assert.equal(await settledSceneSyncs(), syncsBeforeGrid, 'Grid paint only changes the guide, not the mesh/material scene');
  ...
  await page.waitForFunction(() => Object.values(window.cadHarness.state.renderers || {}).some(saved => {
    const lighting = saved.fileSession?.slices?.display?.lighting;
  ...
  // next STEP file), but a mesh resolves them off: nothing is cut, no section appears.
  const assertMeshToolsStayOff = async () => {
    assert.equal((await page.evaluate(() => window.__cadClip())).enabled, false, 'the viewport applies no clip to a mesh');
  ...
  await page.waitForFunction(() => Object.values(window.cadHarness.state.renderers || {}).some(saved => {
    const display = saved.fileSession?.slices?.display;
  ...
  await page.waitForFunction(() => Object.values(window.cadHarness.state.renderers || {}).some(saved => {
    const surfaces = saved.fileSession?.slices?.display?.surfaces;
