...
export const designFeatureLabel = node => node.type === 'profile' ? `Profile · ${node.label === 'RectangleRounded' ? 'Rounded rectangle' : designFeatureTitle(node.label)}` : designFeatureTitle(node.label);
...
export function withDesignFaceRows(nodes, resolved, references, expanded, parentLine = null) {
  return nodes.map(node => {
    ...
    if (['model', 'group', 'parameters', 'parameter', 'sketch', 'profile'].includes(node.type)) return { ...node, children };
...
export function sourceParameterNode(parameter, features) {
  ...
  const visit = (node, parentLine) => {
    const line = ['sketch', 'profile'].includes(node.type) ? parentLine : node.line;
    ...
    if (names.includes(parameter.name)) {
      const collect = (item, inherited) => {
        const current = ['sketch', 'profile'].includes(item.type) ? inherited : item.line;
