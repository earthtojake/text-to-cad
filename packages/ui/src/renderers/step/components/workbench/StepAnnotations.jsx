...
// Copy Reference, and the Annotations section of the file's panel. An annotation reaches the
...
function AnnotationsList({ annotations, canAddToChat, onSelect, onEdit, onRemove, onAddToChat }) {
  const listRef = useRef(null);
  const count = annotations.length;
  const previousCount = useRef(count);
  // A new annotation is the one the person just made: keep it in view.
  useEffect(() => {
    if (count > previousCount.current) listRef.current?.lastElementChild?.scrollIntoView?.({ block: "nearest" });
    previousCount.current = count;
  }, [count]);
  return (
    <ul ref={listRef} className="flex flex-col gap-0.5 px-1 pb-2" aria-label="Annotations">
      {annotations.map((annotation, index) => (
        <AnnotationRow key={annotation.id} annotation={annotation} index={index} canAddToChat={canAddToChat}
          onSelect={() => onSelect(annotation)} onEdit={text => onEdit(annotation.id, text)}
          onRemove={() => onRemove(annotation.id)} onAddToChat={() => onAddToChat(annotation)} />
      ))}
    </ul>
  );
}

/** The panel's Annotations section, or null while there are none. */
export function buildAnnotationsSection({ annotations, canAddToChat, onSelect, onEdit, onRemove, onAddToChat }) {
  ...
  return {
    ...
    title: (
      <span className="flex min-w-0 items-center gap-1.5">
        ...
        <Badge variant="secondary" className="h-4 min-w-4 rounded-full px-1 text-[10px] tabular-nums">{annotations.length}</Badge>
      </span>
    ),
    content: <AnnotationsList annotations={annotations} canAddToChat={canAddToChat}
      onSelect={onSelect} onEdit={onEdit} onRemove={onRemove} onAddToChat={onAddToChat} />
  };
}
