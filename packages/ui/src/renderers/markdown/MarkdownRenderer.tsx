  ...
  if (props.openPanel === SOURCE_PANEL) {
    return (
      <Suspense fallback={<div className="p-4 text-xs text-muted-foreground">Opening source…</div>}>
        <SourceRenderer {...props} data={null} />
      </Suspense>
    );
  }
  return (
    <MarkdownEditor
      content={props.document?.value ?? ""}
      editable={!!props.document && !props.document.readOnly}
      onChange={props.document?.setValue}
      onSave={props.document ? () => void props.document?.save() : undefined}
    />
  );
}

/**
 * A shared markdown file as the document it is, and editable in place.
 *
