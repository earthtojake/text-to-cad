...
class _DeclaredKinematics:
    ...
    block and the bake pose. Choreography is not here — the render module
    beside the document (``<name>.step.js``) is read by the viewer, never by
    a build, so an edit to it is a reload and not a rebuild."""
...
def _resolve_declared_kinematics(defn: object) -> _DeclaredKinematics:
    """The model's kinematics block and bake pose.
    ...
    return _DeclaredKinematics(block=block, bake_pose=bake_pose)
...
def _run_script_generator_body(
    ...
    *,
    ...
    if model_format == "step":
        ...
        # Kinematics + bake pose (validated at decoration) ride the scene into
        # the sidecar.
