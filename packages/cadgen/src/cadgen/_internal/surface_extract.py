...
def _assert_surface_covers_face(payload, u0, u1, v0, v1, bin_out) -> None:
    ...
    # SetNotPeriodic may retain exterior support knots. The evaluator's
    # active span is [knots[degree], knots[pole_count]], not the outer span.
    first_u = knots_u[ku_off + payload["degU"]]
    last_u = knots_u[ku_off + payload["nu"]]
    first_v = knots_u[kv_off + payload["degV"]]
    last_v = knots_u[kv_off + payload["nv"]]
    ...
    if kind in (GeomAbs_SurfaceType.GeomAbs_BSplineSurface,
        ...
        if isinstance(native, Geom_BSplineSurface):
            ...
            _translate_knots_to_window(
                ...
                nurbs.Bounds()[2], nurbs.NbVKnots, nurbs.VKnot, nurbs.SetVKnot)
            active_u0, active_u1, active_v0, active_v1 = nurbs.Bounds()
            if (
                u0 >= active_u0 - eps_u
                and u1 <= active_u1 + eps_u
                and v0 >= active_v0 - eps_v
                and v1 <= active_v1 + eps_v
            ):
