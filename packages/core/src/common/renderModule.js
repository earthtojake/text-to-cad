...
export async function fetchRenderModuleSource(url, { signal } = {}) {
...
export async function loadRenderModule(url, { signal } = {}) {
  signal?.throwIfAborted();
  const source = await fetchRenderModuleSource(url, { signal });
  signal?.throwIfAborted();
  if (source === null) {
    return null;
  }
