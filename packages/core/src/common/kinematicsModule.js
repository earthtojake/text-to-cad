...
async function fetchSidecar(sidecarUrl, signal) {
...
export async function loadKinematicsModuleDefinition(sidecarUrl, { cadPath = "", signal } = {}) {
  signal?.throwIfAborted();
  const sidecar = await fetchSidecar(sidecarUrl, signal);
  signal?.throwIfAborted();
  if (!sidecar) {
    return null;
  }
