...
async function loadPackageMeshData(packageInfo, tessellationCache) {
  ...
  const cachedEntries = await tessellationCache?.getCachedComponentEntries(cids) || new Map();
  const misses = [];
  ...
  const loadComponent = async (cid) => {
    ...
    if (decoded) {
      ...
      await tessellationCache?.writeBackComponentEntry(cid, {}, component, index);
    }
...
export async function loadSource(input, options = {}) {
  ...
  if (!meshData && packageInfo) {
    meshData = await loadPackageMeshData(packageInfo, options.tessellationCache);
