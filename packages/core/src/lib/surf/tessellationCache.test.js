...
let cache = createTessellationCache();
function resetTestCache(provider) {
  cache.dispose();
  cache = createTessellationCache({ provider });
}
...
test("getCachedComponentEntries prefers getMany and falls back to per-key gets", async (t) => {
  ...
  const hits = await cache.getCachedComponentEntries(["hit-a", "miss-b", "hit-c"]);
  ...
  const fallback = await cache.getCachedComponentEntries(["hit-a", "miss-b"]);
...
test("provider: miss tessellates + writes back; hit skips tessellation; unset is a no-op", async (t) => {
  ...
  const first = await cache.tessellateComponentCached(index, floats, { cid: "c0" });
  assert.equal(calls.get, 1);
  assert.equal(calls.put, 1);
  assert.equal(store.size, 1);
  assert.ok(store.has(tessellationCacheKey("c0")));

  const second = await cache.tessellateComponentCached(index, floats, { cid: "c0" });
  assert.equal(calls.get, 2);
  ...
  await cache.tessellateComponentCached(index, floats, { cid: "c0", options: { collectBoundaryDebug: true } });
  assert.equal(calls.get, 2);
  assert.equal(calls.put, 1);

  // No cid — nothing to key on — bypasses too.
  await cache.tessellateComponentCached(index, floats, {});
  assert.equal(calls.get, 2);

  resetTestCache(null);
  const plain = await cache.tessellateComponentCached(index, floats, { cid: "c0" });
...
test("a provider that throws degrades to plain tessellation", async (t) => {
  ...
  const component = await cache.tessellateComponentCached(index, floats, { cid: "c0" });
...
test("priming takes one batch round trip and feeds the per-component reads once", async (t) => {
  ...
  assert.equal(await cache.primeCachedEntryBytes(["hit-a", "miss-b", "hit-a"]), 1);
  assert.equal(calls.getMany, 1);
  // Hits and known misses both answer without a request…
  assert.equal(await cache.getCachedEntryBytes("hit-a"), hit);
  assert.equal(await cache.getCachedEntryBytes("miss-b"), null);
  assert.equal(calls.get, 0);
  // …and each primed answer is taken once: the next read asks the provider.
  assert.equal(await cache.getCachedEntryBytes("hit-a"), null);
  assert.equal(calls.get, 1);
});
...
test("priming without a batch route leaves the per-component reads to the provider", async (t) => {
  ...
  assert.equal(await cache.primeCachedEntryBytes(["a", "b"]), 0);
  await cache.getCachedEntryBytes("a");
  assert.equal(gets, 1);
});
...
test("deferred write-backs queue until quiet, then drain at the configured concurrency", async (t) => {
  t.after(() => {
    resetTestCache(null);
    cache.configureTessellationCacheWriteBack({ deferMs: 0 });
  });
  const puts = [];
  let inFlight = 0;
  let peak = 0;
  ...
  cache.configureTessellationCacheWriteBack({ deferMs: 30, concurrency: 2 });
  const bytes = new Uint8Array([9]);
  await cache.writeBackEntryBytes("c1", {}, bytes);
  await cache.writeBackEntryBytes("c2", {}, bytes);
  await cache.writeBackEntryBytes("c3", {}, bytes);
  await cache.writeBackEntryBytes("c1", {}, bytes); // the same key twice is one write
  ...
  cache.configureTessellationCacheWriteBack({ deferMs: 0 });
  await cache.writeBackEntryBytes("c4", {}, bytes);
...
test("flushing writes the deferred entries now", async (t) => {
  ...
  cache.configureTessellationCacheWriteBack({ deferMs: 10_000, concurrency: 1 });
  await cache.writeBackEntryBytes("c1", {}, new Uint8Array([1]));
  assert.equal(puts.length, 0);
  await cache.flushTessellationCacheWriteBacks();
  assert.equal(puts.length, 1);
});

test('render sessions isolate priming and deferred writes even for the same component key', async () => {
  const written = [[], []];
  const a = createTessellationCache({ provider: {
    getMany: async () => [new Uint8Array([1])], get: async () => null,
    put: async (key, bytes) => written[0].push([...bytes])
  }, writeBack: { deferMs: 10000, concurrency: 1 } });
  const b = createTessellationCache({ provider: {
    getMany: async () => [new Uint8Array([2])], get: async () => null,
    put: async (key, bytes) => written[1].push([...bytes])
  }, writeBack: { deferMs: 10000, concurrency: 1 } });
  await Promise.all([a.primeCachedEntryBytes(['same']), b.primeCachedEntryBytes(['same'])]);
  assert.deepEqual([...await a.getCachedEntryBytes('same')], [1]);
  assert.deepEqual([...await b.getCachedEntryBytes('same')], [2]);
  await a.writeBackEntryBytes('same', {}, new Uint8Array([3]));
  await b.writeBackEntryBytes('same', {}, new Uint8Array([4]));
  a.dispose();
  await b.flushTessellationCacheWriteBacks();
  assert.deepEqual(written, [[], [[4]]]);
  assert.equal(await a.getCachedEntryBytes('same'), null);
  b.dispose();
});
