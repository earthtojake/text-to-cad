...
/** Cache state, priming and write queues belong to one render session. */
/**
 * @param {{provider?: import('./cacheTypes.js').TessellationCacheProvider | null, writeBack?: {deferMs?: number, concurrency?: number}}} configuration
 * @returns {import('./cacheTypes.js').TessellationCache}
 */
export function createTessellationCache({ provider = null, writeBack = {} } = {}) {
  let cacheProvider = provider && typeof provider.get === "function" ? provider : null;
  let disposed = false;

  function tessellationCacheProviderRegistered() {
  ...
  async function getCachedComponentEntry(cid, options = {}) {
  ...
  async function getCachedComponentEntries(cids, options = {}) {
    const hits = new Map();
    const provider = disposed ? null : cacheProvider;
  ...
  async function primeCachedEntryBytes(cids, options = {}) {
    const provider = disposed ? null : cacheProvider;
    ...
    for (let start = 0; start < unique.length; start += PRIME_CHUNK) {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        ...
        if (!disposed) primedEntries.set(keys[i], bytes);
        if (bytes) hits += 1;
      }
    }
    return hits;
  ...
  async function getCachedEntryBytes(cid, options = {}) {
    const provider = disposed ? null : cacheProvider;
    if (!provider || !cid || !tessellationOptionsCacheable(options)) return null;
  ...
  function configureTessellationCacheWriteBack({ deferMs = 0, concurrency = Infinity } = {}) {
    writeBackPolicy = {
  ...
  async function writeBackEntryBytes(cid, options, bytes) {
    const provider = disposed ? null : cacheProvider;
  ...
  async function writeBackComponentEntry(cid, options, component, index) {
    const provider = disposed ? null : cacheProvider;
    if (!provider || typeof provider.put !== "function" || !cid) return;
  ...
  async function tessellateComponentCached(index, floats, { cid = "", options = {} } = {}) {
  ...
  return {
    tessellationCacheProviderRegistered, getCachedComponentEntry, getCachedComponentEntries,
    primeCachedEntryBytes, clearPrimedEntries, getCachedEntryBytes,
    configureTessellationCacheWriteBack, flushTessellationCacheWriteBacks,
    writeBackEntryBytes, writeBackComponentEntry, tessellateComponentCached,
    dispose() {
      if (disposed) return;
      disposed = true;
      cacheProvider = null;
...
export function createHttpTessellationCacheProvider({
  ...
  return {
    async get(key) {
      try {
        const response = await fetchImpl(entryUrl(key), { cache: "no-store", headers, signal });
    ...
    async put(key, bytes) {
      try {
        await fetchImpl(entryUrl(key), { method: "POST", body: bytes, headers, signal });
      } catch {
        // best-effort write-back
      }
    },
