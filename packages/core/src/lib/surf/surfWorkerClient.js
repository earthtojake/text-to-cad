...
import { tessellationOptionsCacheable } from "./tessellationCache.js";
...
export function loadSurfComponentInWorker(url, { signal, tessellation, tessellationCache } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    pendingRequests.set(id, {
      ...
      cacheHit: false,
      writeBack: cacheable
        ? (entryBytes) => { tessellationCache.writeBackEntryBytes(cid, tessellation || {}, entryBytes); }
        : null,
    });
    ...
    if (cacheable) {
      tessellationCache.getCachedEntryBytes(cid, tessellation || {}).then(post, () => post(null));
    } else {
      post(null);
    }
  });
