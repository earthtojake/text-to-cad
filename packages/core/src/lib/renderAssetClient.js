...
export async function loadRenderSurf(url, { signal, tessellationCache } = {}) {
  ...
  const meshData = await loadCached(glbCache, url, async () => {
    return (await loadSurfPayload(url, { signal, tessellationCache })).meshData;
  }, { cachePending: !signal });
  return finalizeCached(glbCache, url, meshData);
}
...
async function loadSurfPayloadInline(url, { signal, tessellation, tessellationCache } = {}) {
  const [
    { parseSurf },
    { tessellateComponent },
    { cidFromSurfUrl },
  ...
  const component = tessellationCache
    ? await tessellationCache.tessellateComponentCached(index, floats, { cid: cidFromSurfUrl(url), options: tessellation || {} })
    : tessellateComponent(index, floats, tessellation || {});
  return {
...
async function loadSurfPayload(url, { signal, tessellation, tessellationCache } = {}) {
  ...
  const payload = await loadCached(surfPayloadCache, cacheKey, async () => {
    ...
    const workerPayload = loadSurfComponentInWorker(url, { signal, tessellation, tessellationCache });
    if (workerPayload) {
      try {
    ...
    return loadSurfPayloadInline(url, { signal, tessellation, tessellationCache });
  }, { cachePending: !signal });
  if (cacheKey !== url) {
 ...
 */
export async function loadRenderSurfPayloadAtLevel(url, { signal, tessellation, tessellationCache } = {}) {
  return loadSurfPayload(url, { signal, tessellation, tessellationCache });
}

export async function loadRenderSurfSelectorBundle(surfUrl, { signal, tessellationCache } = {}) {
  const bundle = await loadCached(selectorCache, surfUrl, async () => {
    return (await loadSurfPayload(surfUrl, { signal, tessellationCache })).bundle;
  }, { cachePending: !signal });
  return finalizeCached(selectorCache, surfUrl, bundle);
}

export async function loadRenderSurfDisplayEdgeBundle(surfUrl, { signal, tessellationCache } = {}) {
  ...
  const bundle = await loadCached(displayEdgeCache, surfUrl, async () => {
    const selector = await loadRenderSurfSelectorBundle(surfUrl, { signal, tessellationCache });
    return {
      manifest: {
