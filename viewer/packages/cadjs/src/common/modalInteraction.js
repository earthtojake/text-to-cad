...
// Free-vibration modal superposition from an initial displacement AND initial
// velocity (so a fast drag/flick imparts kinetic energy, not just a static
// pull). Each mode rings at its own damped natural frequency; physical time is
// slowed by `slowdown` so the motion is analyzable.
//
// Underdamped response per mode (physical time tau):
//   q(tau) = e^{-zeta*w*tau} [ q0 cos(wd*tau) + (v0 + zeta*w*q0)/wd * sin(wd*tau) ]
// with w = 2*pi*f, wd = w*sqrt(1 - zeta^2). v0 is the modal velocity per
// physical second; playback advances tau = t_real / slowdown.
export class ModalSuperposition {
  constructor({ frequencies, weights, velocities = null, damping = 0.02, slowdown = 200 }) {
    this.frequencies = frequencies.slice();
    this.q0 = weights.slice();
    this.v0 = velocities ? velocities.slice() : weights.map(() => 0);
  ...
  weightsAt(tRealSeconds) {
    ...
    return this.q0.map((q0, i) => {
      ...
      const v0 = this.v0[i] || 0;
      const envelope = Math.exp(-zeta * omega * tSim);
      if (omega <= 1e-9) {
        // Rigid / zero-frequency mode: no restoring force, just drift+decay.
        return envelope * (q0 + v0 * tSim);
      }
      const omegaD = omega * Math.sqrt(Math.max(1e-12, 1 - zeta * zeta));
      const b = (v0 + zeta * omega * q0) / omegaD;
      return envelope * (q0 * Math.cos(omegaD * tSim) + b * Math.sin(omegaD * tSim));
    });
  }
