...
`@dxf` takes **only `out=`**. It has no `kind=` (a drawing is 2D geometry) and
no `kinematics=`; passing either is an error. **No decorator takes
...
one** — when it declares kinematics or mesh exports. A plain model that is
geometry and nothing else writes no sidecar at all. Choreography is not in it:
that is the render module beside the document (`<name>.step.js`, step 4).
...
kinematics dict's `"at"` key, `animation=` on any decorator (choreography is
the render module beside the document), `kinematics=` on `@dxf` — or a
