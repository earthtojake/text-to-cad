...
- Suites: `npm --prefix packages/cadgen-js test` (837), `npm --prefix apps/viewer
  run test` (347), `PYTHON_BIN=MAIN/.venv/bin/python scripts/test/test-python.sh`
  (1723),
...
- `221f6c48a` — the hand's render module articulates (see §6).
- `1ca32d56f` — the memory probe accounts for picking bytes, which is how §4
  can now rule picking OUT rather than list it as a suspect.
- `0f05fdf1d` — `cadgen step snapshot --video` renders a clip as an mp4 or a gif
  (see §5), plus the cad edge instance set's free-slot Set.
...
## 5. Rendering a clip as a video

`cadgen step snapshot IN OUT.mp4 --animation <clip> --video '{"fps":60,"quality":"high"}'`
(or `OUT.gif`). The keys are closed: `fps`, `seconds`, `start`, `quality`
(draft/review/high), `loop` (gif only). It needs ffmpeg on PATH or in
`CADGEN_FFMPEG`, checked before the first frame.

Two things worth knowing before you change it. The source, the render module and
the model are prepared ONCE and each frame is one effects pass plus one capture,
so a 1,800-frame render costs one load — do not let a refactor put `loadSource`
back inside the frame loop. And the camera is fitted ONCE to the union of every
frame's bounds and then held, because a still's per-pose auto-fit is exactly the
breathing a video must not have; the stage floor and grid are sized from those
same union bounds.

The shot used for the hand:

```bash
cadgen step snapshot assemblies/STEP/anthropomorphic_hand/hand_mechanical_candidate_r13.step \
  showcase.mp4 --animation showcase --video '{"fps":60,"quality":"high"}' \
...
one fix for both, not a video regression.

## 6. The hand's render module
