...
await build({
  ...
  plugins: [{ name: 'host-boundaries', setup(plugin) {
    ...
    plugin.onResolve({ filter: /^@hardcore\/ui\/file-viewer$|^@hardcore\/ui\/renderers\/(cad|glb|mesh)$|^@hardcore\/ui\/renderers\/cad\/(presentation|empty)$|ViewerTopBar\.jsx$|useViewerAutoReload\.js$/ }, args => ({ path: args.path, namespace: 'host-test' }));
...
test('web host preserves compact navigation, history, root state and focus refresh lifecycle', async () => {
  ...
  try {
    ...
    assert.deepEqual(snapshot().renderers.map(renderer => renderer.id), ['cad', 'glb', 'mesh']);
