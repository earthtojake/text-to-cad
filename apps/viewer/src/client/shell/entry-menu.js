 ...
 *
 * Pure, and imports only `cadgen-js`, so `node --test` loads it without the
 * bundler's aliases.
 */
import { isCadFile } from "cadgen-js/lib/fileFormats.js";
...
function fileSections({ target, platform, rename, trash, copies }) {
  ...
  return [
    ...
    // the token the agent reads. Which files those are is `cadgen-js`.
