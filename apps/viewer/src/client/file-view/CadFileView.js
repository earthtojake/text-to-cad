...
} from "../components/workbench/ThemeSettingsPopover";
import MeshFileSheet from "../components/workbench/MeshFileSheet";
...
import ViewerAlertDialog from "../components/workbench/ViewerAlertDialog";
import ViewerLoadingOverlay from "../components/workbench/ViewerLoadingOverlay";
import {
...
import { useCadDirectorySession } from "../components/workbench/hooks/useCadDirectorySession";
...
import { toFiniteNumber } from "@/workbench/valueUtils";
import {
...
import { CAD_WORKSPACE_LAYOUT_MODE } from "@/workbench/breakpoints";
import {
...
import { cadApiUrl, getCadManifestStore } from "../workbench/cadManifestStore.js";
import {
  FILE_STATUS_LEVELS,
...
import {
  ...
  readInitialFileSheetWidthIsCustom,
...
// THE per-file surface: everything the CAD Viewer draws for ONE file, and
// nothing about which files exist.
//
// `file` is the served-root-relative path (what the standalone app keeps in
// `?file=`); `onOpenFile(path, { history })` is how the surface asks to be
// pointed somewhere else — a linked STEP, an entry picked out of injected
...
function CadFileViewSurface({
  ...
  // `file`. Both read the same store, so subscribing here is never a second
  // source of truth.
  const manifestStore = getCadManifestStore(origin);
  ...
  const resolvedCatalog = catalog || {
    entries: storeSnapshot.manifest.entries,
    revision: storeSnapshot.revision,
    hydrated: storeSnapshot.catalogHydrated,
    refreshing: storeSnapshot.catalogRefreshing,
    error: storeSnapshot.catalogError,
  };
  const manifestEntries = Array.isArray(resolvedCatalog.entries) ? resolvedCatalog.entries : [];
  const manifestRevision = Number(resolvedCatalog.revision) || 0;
  const catalogHydrated = resolvedCatalog.hydrated === true;
  ...
  // know a URL exists.
  const readCadParam = useCallback(() => explicitFileParam, [explicitFileParam]);
  const writeCadParam = useCallback((urlPath, options) => {
    onOpenFile?.(normalizeCadFileQueryParam(urlPath), options || {});
  }, [onOpenFile]);
  const selectedEntryKeyFromUrl = useCallback(
    (entries) => selectedEntryKeyForFile(entries, explicitFileParam),
    [explicitFileParam]
  );
  // Tell this origin's catalog store which file to name on its next request:
  // the standalone store reads the page's own `?file=`, and an embedded one has
  // no such URL to read.
  useEffect(() => {
    manifestStore.setFileHint(explicitFileParam || "");
  }, [explicitFileParam, manifestStore]);
  ...
  //
  // "" is the standalone app, whose localStorage IS the backend's origin, so the
  // namespace was already exactly this. An embedded surface stores under the
  ...
  useEffect(() => {
    ...
    let active = true;
    fetch(cadApiUrl("/__cad/server", { origin, file: explicitFileParam || "" }), {
      cache: "no-store",
  ...
  }, [catalogRootDir, explicitFileParam, origin]);
  ...
  const handleCopyFileAssetReference = useCallback(async (entry, asset = "output", assetInfo = null, referenceKind = "path") => {
    ...
    try {
      ...
      if (kind === "filename") {
        ...
        // A pasteable link points at the VIEWER. Standalone that is the origin in
        // the user's address bar, because the page and the backend are the same
        // server; embedded it is the backend's own origin, which is the only one
        // the link can be opened at. The server's self-reported URL stands in when
        // there is neither.
        const linkOrigin = origin ||
          (typeof window !== "undefined" && window.location?.origin) ||
          String(viewerServerInfo?.url || "").trim();
        copyText = viewerDeepLinkForFileAccessAsset(resolvedAsset, viewerServerInfo, { origin: linkOrigin });
