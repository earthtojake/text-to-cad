/**
 * The contract's vocabulary: what a channel is, and how the three processes
 * derive their view of one.
 *
 * Separate from `index.ts` so a branch file (`explorer.ts`, `cad.ts`) can
 * declare channels without importing the module that assembles them all — the
 * cycle that would otherwise exist between the map and its parts.
 */
