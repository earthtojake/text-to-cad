 ...
 * lists them; the cross takes them all back out of the draft. They go out with the prompt when
 * the person sends it (`withAnnotations`).
 */
export function AnnotationsChip({ annotations, onRemove }: { annotations: DraftAnnotation[]; onRemove: () => void }) {
      ...
      <HoverCard closeDelay={100} openDelay={150}>
        <HoverCardTrigger asChild>
          <span className="inline-flex h-8 max-w-full cursor-default items-center gap-1.5 rounded-lg border bg-muted/30 pr-1 pl-2 text-[12px]">
            <MessageSquareText aria-hidden className="size-3.5 text-muted-foreground" />
        ...
        <HoverCardContent align="start" className="w-80 p-2" side="top">
          <ol aria-label="Annotations in this draft" className="flex flex-col gap-1.5 text-[12px] leading-5">
            {annotations.map((annotation, index) => (
              <li className="flex gap-1.5" key={annotation.id}>
                ...
                <span className="min-w-0">
                  {annotation.references.map((reference) => (
                    ...
                    >
                      <Hash aria-hidden className="size-2.5 shrink-0 text-muted-foreground" />
                      <span className="truncate">{reference.label ?? reference.text}</span>
                    </span>
                  ))}
 ...
 */
export function withAnnotations(text: string, annotations: DraftAnnotation[]): string {
  if (annotations.length === 0) {
    return text;
  }
  const lines = annotations.map((annotation, index) => {
    const references = annotation.references
      .map((reference) => (reference.label ? `${reference.text} (${reference.label})` : reference.text))
      .join(" ");
