...
test("the composer starts at one row, grows, and caps at eight", async () => {
  const userTurnsBefore = await page.locator("[data-turn][data-role=user]").count();
  ...
  // Shift+Enter grows the draft without creating a turn, even when this
  // test runs on its own before any conversation has been sent.
  await expect(page.locator("[data-turn][data-role=user]")).toHaveCount(userTurnsBefore);
  await expect(page.locator("[data-composer] form [data-chip]")).toHaveCount(0);
