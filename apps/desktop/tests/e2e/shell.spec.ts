...
/**
 * No selected session, no explorer. The strip belongs to a session: with none
 ...
 */
test("shows two panes and no explorer until a session is selected", async () => {
...
test("the explorer opens and closes a tab", async () => {
  // A strip needs a session. `tests/e2e/explorer.spec.ts` covers what each
  ...
  // Selecting a session brings the toggle back with that session's own state — closed,
  ...
  // session nobody has opened it in, and a closed pane is not in the document.
  ...
  // deleting the last session also removes the derived directory group.
