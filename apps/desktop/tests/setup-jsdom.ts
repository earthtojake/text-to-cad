...
Object.defineProperty(window, "hardcore", {
  ...
  value: {
    ...
    explorer: {
      ...
      createFile: vi.fn(async ({ path, name }: { path: string; name: string }) => ({ status: "committed", path: path ? `${path}/${name}` : name, change: { kind: "added", path: path ? `${path}/${name}` : name, directory: false } })),
      createDirectory: vi.fn(async ({ path, name }: { path: string; name: string }) => ({ status: "committed", path: path ? `${path}/${name}` : name, change: { kind: "added", path: path ? `${path}/${name}` : name, directory: true } })),
      rename: vi.fn(async ({ path, name }: { path: string; name: string }) => ({
        status: "committed", path: [...path.split("/").slice(0, -1), name].join("/"),
        change: { kind: "moved", previousPath: path, path: [...path.split("/").slice(0, -1), name].join("/"), directory: false },
      })),
      duplicate: vi.fn(async ({ path }: { path: string }) => ({ status: "committed", path: `${path} copy`, change: { kind: "added", path: `${path} copy`, directory: false } })),
      trash: vi.fn(async ({ path }: { path: string }) => ({ status: "committed", path, change: { kind: "removed", path, directory: false } })),
