...
beforeEach(() => {
  ...
  useExplorer.setState({ sessionId: projectId, projectId, root: null, tabs: [], activeId: null, ready: true });
  useSessions.setState({ sessions: [projectId, "other"].map(id => ({ id, projectId: id, cwd: `/projects/${id}` })) as Session[], activeId: projectId });
  useUi.setState({ route: 'app' });
});
afterEach(() => { useExplorer.getState().discardSessionResources(projectId); vi.runOnlyPendingTimers(); vi.useRealTimers(); });

it('opens an empty temporary canvas and reports identity without an editable scene', async () => {
  const root = '/worktrees/sketch';
  const result = await performIntegrationCommand({ sessionId: projectId, requestId: 'r', kind: 'open-drawing', projectId, root, title: 'Plan' }) as { tabId: string };
  expect(getDrawingScene(result.tabId)).toBeNull();
  expect(await performIntegrationCommand({ sessionId: projectId, requestId: 'r', kind: 'drawing-state', projectId, root, tabId: result.tabId })).toEqual({
    tabId: result.tabId, title: 'Plan', root, ephemeral: true, elementCount: 0,
  });
  expect(await performIntegrationCommand({ sessionId: projectId, requestId: 'r', kind: 'list-tabs', projectId, root })).toMatchObject({
...
it('reads and captures a background drawing without changing project, tab, root or Settings', async () => {
  ...
  const command = { sessionId: projectId, requestId: 'r', projectId, root: '/worktrees/sketch', tabId: tab.id };
...
it('refuses other projects, other roots and closed drawing tabs before capture', async () => {
  ...
  const base = { sessionId: projectId, requestId: 'r', kind: 'drawing-capture' as const, projectId, tabId: tab.id };
  await expect(performIntegrationCommand({ ...base, root: null })).rejects.toThrow(/closed|workspace|session/);
  await expect(performIntegrationCommand({ ...base, projectId: 'other', root: '/worktrees/sketch' })).rejects.toThrow(/closed|workspace|session/);
...
it('renames an inactive drawing without switching projects or accepting a different root', async () => {
  const tab = useExplorer.getState().open('drawing', { root: '/worktrees/sketch', title: 'Plan' })!;
  await useExplorer.getState().bindSession('other', 'other');
  const command = { sessionId: projectId, requestId: 'r', projectId, tabId: tab.id, root: '/worktrees/sketch', kind: 'drawing-rename' as const, title: '  Revised plan  ' };
  await expect(performIntegrationCommand({ ...command, root: null })).rejects.toThrow(/closed|workspace|session/);
